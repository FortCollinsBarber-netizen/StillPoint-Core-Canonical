...
class JubileeCompletionTests(unittest.TestCase):
    ...
    def test_seven_sabbatical_thresholds_end_at_year_49(self):
        ...
        for year in range(1, 50):
            state = jubilee_state(
                common_year=2000 + year - 1,
                epoch_common_year=2000,
            )
    ...
    def test_jubilee_never_rewrites_annual_law(self):
        ...
        for common_year in range(2000, 2101):
            jubilee_state(
        ...
        self.assertEqual(
            after["ordinaryCalendar"]["baseYearDays"],
            364,
        )
        self.assertEqual(
