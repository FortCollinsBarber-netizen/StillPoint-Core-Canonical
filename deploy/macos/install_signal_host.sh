...
RELEASES="$APP_SUPPORT/releases"
CURRENT="$APP_SUPPORT/current-signal"
...
mkdir -p "$APP_SUPPORT" "$RELEASES" "$RUNTIME/state" "$BIN" "$LOGDIR" "$HOME/Library/LaunchAgents"
...
if [ -f "$RELEASE/.ready" ]; then
  ...
  test -x "$VENV/bin/stillpoint-signal-mail" || die "release marker exists without Signal runtime"
...
STILLPOINT_ROOT="$RUNTIME" STILLPOINT_SOURCE_ROOT="$CORE" \
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.stillpoint.signal-mail</string>
  <key>ProgramArguments</key>
  <array><string>$BIN/run_signal_mail.sh</string><string>serve</string></array>
  <key>RunAtLoad</key><true/>
  <key>KeepAlive</key><true/>
  <key>ThrottleInterval</key><integer>60</integer>
  <key>ProcessType</key><string>Background</string>
  <key>StandardOutPath</key><string>$LOGDIR/signal-mail.out.log</string>
  <key>StandardErrorPath</key><string>$LOGDIR/signal-mail.err.log</string>
</dict>
</plist>
PLIST
chmod 600 "$PLIST"
...
echo "==> Candidate release Keychain/readiness preflight"
STILLPOINT_RELEASE_ROOT="$RELEASE" "$BIN/run_signal_mail.sh" check

echo "==> Atomically select release for Signal"
