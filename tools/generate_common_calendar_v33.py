...
def sunset_utc(civil_date: dt.date, latitude: float, longitude: float) -> dt.datetime:
    return apparent_sunset_utc(
        civil_date,
        GeoPoint(latitude, longitude, "PUBLICATION_REFERENCE"),
    )
...
def equinox_map(document: dict[str, Any]) -> dict[int, dt.datetime]:
    return {
        int(row["year"]): parse_utc(row["instantUTC"])
...
def choose_reconciliation(
    *,
    ...
    chosen_gate = (
        ...
        else decision.delayed_target_date
    )
        ...
        years.append(
            {
                ...
                "governingMarchEquinoxUTC": equinox.isoformat().replace(
                    "+00:00",
                    "Z",
                ),
...
def main() -> None:
    ...
    if latitude is None or longitude is None:
        raise SystemExit(
            "referencePoint latitude/longitude are required in private generation config."
        )
    custody_nonce = point.get("custodyNonce")
    if custody_nonce is None:
        raise SystemExit("referencePoint.custodyNonce is required")

    authority = ref.get("publicationAuthority")
    if not isinstance(authority, dict):
        raise SystemExit("publicationAuthority is required")
    authority_id = authority.get("id")
    authority_status = authority.get("status")
    if not isinstance(authority_id, str) or not authority_id.strip():
        raise SystemExit("publicationAuthority.id is required")
    if authority_status not in ("pilot", "enacted"):
        raise SystemExit(
            "publicationAuthority.status must be either pilot or enacted"
        )
    ...
    output = generate(
        ...
        reference_point_id=str(point.get("id") or "UNSPECIFIED_REFERENCE"),
    )
