...
class OfficeRuntimeCoordinator:
    ...
    def initialize_offices(self,*,now_iso=None):
        ...
        c.commit()
    def _event(self,*,role,worker_id,event_type,occurred_at,error="",metadata=None):
        self.db._connection().execute("""INSERT INTO office_runtime_events
    ...
    def worker_started(self,role,worker_id,*,now_iso=None):
        ...
        now_iso=now_iso or _now(); c=self.db._connection(); row=c.execute("SELECT * FROM office_runtime_state WHERE role=?",(role,)).fetchone()
        if not row: self.initialize_offices(now_iso=now_iso); row=c.execute("SELECT * FROM office_runtime_state WHERE role=?",(role,)).fetchone()
        ...
        self._event(role=role,worker_id=worker_id,event_type=et,occurred_at=now_iso); c.commit()
    def observe_health(self,role,*,worker_id,heartbeat_at,healthy,error="",now_iso=None):
        ...
        if old!=new: self._event(role=role,worker_id=worker_id,event_type=new,occurred_at=now_iso,error=error)
        c.commit()
    def worker_stopped(self,role,worker_id,*,now_iso=None):
        ...
        self._event(role=role,worker_id=worker_id,event_type='stopped',occurred_at=now_iso); c.commit()
    ...
    def _handoff_event(self,*,task_id,from_role,to_role,event_type,reason,occurred_at,metadata=None):
        self.db._connection().execute("""INSERT INTO task_office_handoff_events(event_id,task_id,from_role,to_role,event_type,reason,occurred_at,metadata_json) VALUES(?,?,?,?,?,?,?,?)""",
    ...
    def assign_task(self,task_id,owner_role,*,assigned_by,reason,now_iso=None,event_type='assigned'):
        ...
        now_iso=now_iso or _now(); c=self.db._connection(); cur=c.execute("SELECT * FROM task_office_assignments WHERE task_id=?",(task_id,)).fetchone()
        if cur:
            if cur['owner_role']==owner_role and cur['state']=='active': return dict(cur)
        ...
        self._handoff_event(task_id=task_id,from_role=None,to_role=owner_role,event_type=event_type,reason=reason,occurred_at=now_iso,metadata={'assigned_by':assigned_by}); c.commit(); return self.get_assignment(task_id)
    def handoff_task(self,task_id,*,from_role,to_role,reason,now_iso=None):
        ...
        now_iso=now_iso or _now(); c=self.db._connection(); row=c.execute("SELECT * FROM task_office_assignments WHERE task_id=?",(task_id,)).fetchone()
        ...
        self._handoff_event(task_id=task_id,from_role=from_role,to_role=to_role,event_type='handoff',reason=reason,occurred_at=now_iso,metadata={'handoff_count':count+1}); c.commit(); return self.get_assignment(task_id)
    def release_task(self,task_id,*,owner_role,reason,now_iso=None):
        now_iso=now_iso or _now(); c=self.db._connection(); row=c.execute("SELECT * FROM task_office_assignments WHERE task_id=?",(task_id,)).fetchone()
        if not row or row['state']!='active' or row['owner_role']!=owner_role: return
        c.execute("UPDATE task_office_assignments SET state='released',updated_at=? WHERE task_id=?",(now_iso,task_id))
        self._handoff_event(task_id=task_id,from_role=owner_role,to_role=owner_role,event_type='released',reason=reason,occurred_at=now_iso); c.commit()
